...
//     await SeatLock.deleteMany({ expiredAt: { $lte: new Date() } });
// };
...
//     const soldSeatIds = [
//         ...confirmedBookings.flatMap((b) => b.seats.map((s) => s.toString())),
...
//             (l) => l.userId.toString() === req.user.Iid.toString()
//         );
...
//             myLocks.length > 0
//                 ? new Date(
//                     Math.min(...myLocks.map((l) => new Date(l.expiredAt).getTime()))
//                 )
//                 : null;
//         res.json({
//             soldSeatIds,
...
//             Showtime.findById(seatId).select("screenId"),
//         ]);
...
//             return res.status(400).json({ message: "Seat does not belong to this showtime screen" });
//         }

//         //sold check
//         const [bookingConfilct, ticketConflict] = await Promise.all([
//             Booking.exists({
//                 showtime: showtimeId,
//                 status: "confirmed",
//                 seats: seatId,
